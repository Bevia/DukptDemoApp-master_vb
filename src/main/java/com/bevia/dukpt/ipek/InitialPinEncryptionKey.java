package com.bevia.dukpt.ipek;

import com.bevia.dukpt.utils.TripleDesKeyValidator;
...
public class InitialPinEncryptionKey {
    ...
    private byte[] generateIPEK(BaseDerivationKey baseDerivationKey, String ksn) {
        ...
        String bdk = XorHelper.xorKeyComponents(
                new BinaryKeyComponent(component0),
        ...
        From standard: 0123456789ABCDEF FEDCBA9876543210   (32 hexidecimal characters)
        From here:     70ABE3518528FDCB 6F6C168370A5E29B   (32 hexidecimal characters)
        bdk24Byte:     70ABE3518528FDCB 6F6C168370A5E29B 70ABE3518528FDCB
    ...
    private List<HexKeyComponent> createKeyComponents(byte[] bdk){
        ...
        HexKeyComponent component1 = new HexKeyComponent(
                Hex.toHexString(rawKey1).toUpperCase(),
                TripleDesKeyValidator.createSmallCheckValue(Hex.toHexString(rawKey1)));

        byte[] rawKey2 = new byte[bdk.length];
        rand.nextBytes(rawKey2);
        HexKeyComponent component2 = new HexKeyComponent(
                Hex.toHexString(rawKey2).toUpperCase(),
                TripleDesKeyValidator.createSmallCheckValue(Hex.toHexString(rawKey2)));
        byte[] intermediateValue = xor(rawKey1, rawKey2);

        byte[] rawKey3 = xor(intermediateValue, bdk);
        HexKeyComponent component3 = new HexKeyComponent(
                Hex.toHexString(rawKey3).toUpperCase(),
                TripleDesKeyValidator.createSmallCheckValue(Hex.toHexString(rawKey3)));
